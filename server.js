...
// FIX: per-listener ICY metadata support
...
function loadState() {
  try {
    ...
    if (fs.existsSync(newsFile)) {
      ...
      news = [{ id: 'n1', date: new Date().toISOString(), status: 'news', title: 'BUKUMA RADIO GOES GLOBAL',
                summary: 'Agum Bukuma Radio now streams to the world.', content: 'We are live.' }];
      saveNews();
    }
    // Self-heal: reset stuck statuses and missing files
    queue.forEach(t => {
...
// Scout: pre-fetch next 3 tracks ahead of pointer every 10s
let scoutTimer = null;
function startScout() {
...
// ── The Player ─────────────────────────────────────────────────────────────────
// CORE RADIO RULES:
// 1. playTrack() is the ONE function that spawns FFmpeg.
// 2. ALWAYS clear playNextTimeout at entry — prevents double-spawn from stale timers.
// 3. FFmpeg close event: only advance if bytesOut > threshold (guards against instant-exit).
// 4. Error tracks: batch-scan forward to next non-error track in one pass.
// 5. Stream clients: write directly; manage back-pressure; NEVER restart FFmpeg for new listeners.
// 6. Volume changes: do NOT restart FFmpeg — clients get the new volume on next track.
// 7. New listener connects: they get the live stream mid-song — no restart triggered.

function playTrack() {
  // FIX A: always cancel any pending retry timer first — prevents double-spawn
  ...
  if (track.status !== 'ready' || !track.localPath || !fs.existsSync(track.localPath)) {
    ...
    if (track.status === 'error') {
      // FIX B: batch-scan forward in one pass — no rapid-fire chain skips
      ...
      // Scan forward to first non-error track
      ...
      console.log(`[PLAYER] Skipping ${scanned + 1} errored track(s), jumping to index ${next}`);
      libraryIndex = next;
      saveState();
      playNextTimeout = setTimeout(playTrack, 500);
      return;
    }
    // status='ready' but file missing — re-download
  ...
  proc.stdout.on('data', chunk => {
    ...
    // FIX C: broadcast to all stream listeners without restarting anything
    const dead = [];
    streamClients.forEach(client => {
      try {
        // FIX D: back-pressure guard — drop slow clients rather than blocking FFmpeg
        const backlog = (client.socket && client.socket.writableLength) || 0;
        if (backlog > 2 * 1024 * 1024) { // 2MB backlog = client is too slow
          console.warn('[STREAM] Dropping slow client (backlog:', backlog, ')');
          dead.push(client);
          return;
        }
  ...
  proc.on('close', (code) => {
    if (currentProcess !== proc) return; // stale — a skip/stop killed this
    currentProcess = null;
    console.log(`[PLAYER] Finished: ${currentTrack ? currentTrack.title : '?'} (${bytesOut} bytes, code ${code})`);
    if (!isPlaying) return;

    // FIX E: if FFmpeg exited instantly with almost no bytes, it's a bad file — retry same track
    if (bytesOut < 8192) {
      console.warn(`[PLAYER] Suspiciously short play (${bytesOut} bytes) — marking error and skipping`);
    ...
    saveState();
    broadcast(getStatus());
    // FIX F: 500ms gap prevents audio players from detecting a stream cut
    playNextTimeout = setTimeout(playTrack, 500);
  });

  // FIX G: FFmpeg error — retry same track, never advance
  proc.on('error', err => {
    if (currentProcess !== proc) return;
    currentProcess  = null;
    isStartingTrack = false;
    console.error('[PLAYER] FFmpeg error (retrying same track):', err.message);
    if (!isPlaying) return;
    playNextTimeout = setTimeout(playTrack, 3000);
  });
...
function skipTrack() {
  // FIX H: clear timer FIRST so close-event from killed proc can't chain-fire playTrack
  if (playNextTimeout) { clearTimeout(playNextTimeout); playNextTimeout = null; }
  const old = currentProcess;
  currentProcess  = null;
  isStartingTrack = false;
...
// Volume: update — does NOT restart FFmpeg (takes effect on next track)
// FIX I: restarting FFmpeg on volume change was causing the "skip" — removed entirely
...
function dropJingle(jingleFile = 'ident.mp3') {
  ...
  const args = ['-hide_banner', '-loglevel', 'error', '-i', p, '-f', 'mp3', '-b:a', '128k', 'pipe:1'];
...
// ── Stream handler ─────────────────────────────────────────────────────────────
// FIX J: New listeners attach to the live stream MID-SONG — no FFmpeg restart.
// This is the core of a radio server. The stream is a push pipe; clients join/leave freely.
function streamHandler(req, res) {
  ...
  // ICY headers for Winamp / VLC / dedicated radio players
  ...
  console.log(`[STREAM] New listener connected. Total: ${streamClients.size}`);
}
...
wss.on('connection', ws => {
  ...
  ws.on('message', (data, isBinary) => {
    // FIX K: binary data = mic PCM from admin — ignore for now (stub)
    if (isBinary) return;
    try {
...
app.delete('/api/queue/:id', requireAuth, (req, res) => {
  ...
  saveState();
  if (wasCurrentlyPlaying && isPlaying) skipTrack(); else broadcast(getStatus());
  res.json({ success: true });
});
...
// Playlist routes (both /api/playlists and /api/admin/playlists)
...
app.post('/api/admin/drive/play', requireAuth, (req, res) => {
  ...
  const track = { id: Math.random().toString(36).slice(2), status: 'ready', title: path.basename(filePath, path.extname(filePath)), artist: 'Drive', youtubeQuery: 'LOCAL', localPath: filePath };
...
server.listen(PORT, () => {
  ...
  setTimeout(() => {
    if (isPlaying && queue.length > 0 && !currentProcess && !isStartingTrack) {
      console.log('[BOOT] Resuming playback from saved state');
      playTrack();
    }
  }, 5000);
});
