...
let isStartingTrack= false;   // guard: only one FFmpeg at a time
...
function loadState() {
              try {
                              ...
                              if (fs.existsSync(stateFile)) {
                                                ...
                                                // NOTE: do NOT restore isPlaying — always start stopped, boot resume handles it
                ...
                queue.forEach(t => {
                              ...
                              console.error('[INIT] loadState error:', e.message);
              }
}
...
function getStatus() {
              return {
                              type: 'status',   // FIX #13: always tag so clients can match on type
                              isPlaying,
                              volume,
...
// ── The Player ────────────────────────────────────────────────────────────────
// RULES:
//  - playTrack() is the ONE AND ONLY function that spawns FFmpeg
//  - Only FFmpeg's natural close event OR explicit user skip/stop may advance
//  - Error event: retry same track, do NOT advance
//  - Error-status tracks: skip forward, but only those
//  - No watchdog. No timeout recovery loops. No double-spawn.

function playTrack() {
              // FIX #2 / #8: single unified guard
  ...
  // Track file not ready yet
  if (track.status !== 'ready' || !track.localPath || !fs.existsSync(track.localPath)) {
                  if (track.status === 'pending') enqueueDownload(track);

                if (track.status === 'error') {
                                  // Only skip error tracks — check if ALL remaining tracks are errors to avoid infinite loop
                    const nonErrorExists = queue.some(t => t.status !== 'error');
                                  if (!nonErrorExists) {
                                                      console.log('[PLAYER] All tracks errored — waiting for scout to recover them');
                                                      // Reset them all to pending so scout retries
  ...
  let proc;
              try {
                              ...
                              // FIX #3: reset flag so player can retry
  ...
  let bytesOut = 0;
              proc.stdout.on('data', chunk => {
                              ...
                              streamClients.forEach(client => {
                                                try {
                                                                    // FIX #7: correct back-pressure check — use socket.writableLength
  ...
  proc.on('close', (code) => {
                  if (currentProcess !== proc) return;  // stale — skip/stop killed this
              currentProcess = null;
                  console.log(`[PLAYER] Finished: ${currentTrack ? currentTrack.title : '?'} (${bytesOut} bytes, code ${code})`);
                  if (!isPlaying) return;  // user stopped — do not advance
              // Natural end: advance and play next
              libraryIndex = (libraryIndex + 1) % queue.length;
                  saveState();
                  broadcast(getStatus());
  ...
  // FIX #1: error event retries same track — does NOT advance
  proc.on('error', err => {
                  ...
                  if (!isPlaying) return;
                  // Retry same track after brief pause — do NOT increment libraryIndex
              playNextTimeout = setTimeout(playTrack, 3000);
  });
}

function startPlayback() {
              // FIX #8: guard on isPlaying alone — currentProcess may be null while waiting
  if (isPlaying) return;
              isPlaying = true;
...
// Volume: update and save — does NOT restart current track
...
// FIX #6: jingle pauses main stream, plays jingle, then resumes — no interleave corruption
function dropJingle(jingleFile = 'ident.mp3') {
  ...
  // Pause the main FFmpeg stdout during jingle
  ...
  activeJingleProcess.on('close', () => {
                  ...
                  // Resume the main stream after jingle finishes
                             if (currentProcess) {
                                               try { currentProcess.stdout.resume(); } catch(e) {}
                             }
  });
}
...
app.post('/api/herald', requireAuth, (req, res) => {
              ...
              news.unshift(item); saveNews();
              res.json({ success: true, item });
});
app.delete('/api/herald/:id', requireAuth, (req, res) => {
              news = news.filter(n => n.id !== req.params.id); saveNews();
              res.json({ success: true });
});

app.get('/api/vault', (req, res) => {
              const pl = playlists[0] || { tracks: [] };
              res.json({ tracks: pl.tracks.map(t => ({
                              id: t.id, title: t.title, artist: t.artist,
                              duration: t.duration, thumbnail: t.thumbnail, status: 'ready'
              })) });
});

// FIX #11: unified playlist routes that respond to BOTH /api/playlists AND /api/admin/playlists
...
// FIX #11: programs endpoints (stored in state)
let programs = [];
...
app.post('/api/admin/programs/reset', requireAuth, (req, res) => {
              // Simply restart from beginning of queue
           libraryIndex = 0; saveState();
              res.json({ success: true });
});

// FIX #11: drive play endpoint
app.post('/api/admin/drive/play', requireAuth, (req, res) => {
              ...
              // Insert at next position and skip to it
           const insertIdx = libraryIndex + 1;
              queue.splice(insertIdx, 0, track);
              libraryIndex = insertIdx;
              const old = currentProcess; currentProcess = null; isStartingTrack = false;
              if (old) { old.removeAllListeners(); try { old.kill('SIGKILL'); } catch(e) {} }
              if (playNextTimeout) { clearTimeout(playNextTimeout); playNextTimeout = null; }
              isPlaying = true; saveState(); broadcast(getStatus()); playTrack();
              res.json({ success: true });
});

// FIX #11: mic duck stub (no real mic mixing — just acknowledge)
...
server.listen(PORT, () => {
              ...
              if (autoJingles.start) startAutoJingleLoop();
                ...
                // FIX #2 / #5: boot resume — only call playTrack() directly, only if nothing running
                // isPlaying is NOT restored from disk (see loadState), so this only fires if
                // the admin explicitly re-starts. Left here as a safety net for future use.
                setTimeout(() => {
